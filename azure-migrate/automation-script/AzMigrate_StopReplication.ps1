...
#TODO - Most of the logError should also return. please check this in all scripts
...
Function ProcessItemImpl($processor, $csvItem, $reportItem) {
    ...
    if((-not $ReplicatingServermachine) -or ($csvItem.OK_TO_MIGRATE -ne 'Y') -or ($csvItem.OK_TO_CLEANUP -ne 'Y')){ `
        ...
        $processor.Logger.LogError("We cannot initiate stop replication as either is it not configured in csv file OR the state of this machine replication is not suitable for initiating Migration Now: '$($sourceMachineName)'")
    ...
    if (-not $StopReplicationJob){
        $processor.Logger.LogError("Stopping the Migration couldn't be initiated for the specified machine: '$($sourceMachineName)'")        
    }
    else {
        $processor.Logger.LogTrace("Stopping the Migration Job is initiated for the specified machine: '$($sourceMachineName)'")
        $reportItem.AdditionalInformation = "Stopping the Migration Job is initiated for the specified machine: '$($sourceMachineName)'"
    }    
}
